...
function flagship_get_entry_comments_link( $args = array() ) {
	...
	$defaults = apply_filters( 'flagship_entry_comments_link_defaults',
		array(
			...
			'before'      => '',
			'hide_if_off' => 'enabled',
			'more'        => __( '% Comments', 'hoaloha' ),
			'one'         => __( '1 Comment', 'hoaloha' ),
			'zero'        => __( 'Leave a Comment', 'hoaloha' ),
		)
	);
	$args = wp_parse_args( $args, $defaults );
	...
	while ( $counter <= $footer_widgets ) {
		hybrid_register_sidebar(
			array(
				...
				'name'        => sprintf( __( 'Footer %d', 'hoaloha' ), $counter ),
				'description' => sprintf( __( 'Footer %d widget area.', 'hoaloha' ), $counter ),
			)
		);
